      ...
      <v-col cols="12">
        <div class="legal-content">
          <div v-if="loading" class="text-center py-8">
            ...
            <p class="mt-4 text-subtitle-1">Loading Security Policy...</p>
          </div>
          
          <div v-else-if="error" class="text-center py-8">
            <v-icon size="48" color="error" class="mb-4">mdi-alert-circle</v-icon>
            <h2 class="text-h5 mb-2">Failed to load Security Policy</h2>
...
const loadMarkdown = async () => {
  ...
  try {
    const response = await fetch('/docs/security-policy.md');
    
    if (!response.ok) {
      throw new Error(`Failed to load security policy: ${response.status} ${response.statusText}`);
    }
